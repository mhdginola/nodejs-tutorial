...
export class ConsoleKernel {
   ...
   */
  async register() {
    // const result = await fileSearch("./commands/*.command.(js|ts)", this.path, { maxDeep: 2, regExp: true });
    // for (let i = 0; i < result.length; i++) {
    const { default: Command } = await import(`./commands/db-seed/index.command.js`);
